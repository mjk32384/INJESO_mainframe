...
void Quaternion_toEulerZYX2(Quaternion* q, double output[3])
{
	assert(output != NULL);
	
	// Roll (z-axis rotation)
	double sinr_sinp = q->v[2];
	double cosr_sinp = q->w;
	output[2] = 2*atan2(sinr_sinp, cosr_sinp);
	//Yaw (y-axis direction)

	output[0] = atan2( q->v[1], q->v[0])+output[2]/2.0;

	// Pitch (y-axis rotation)
	double cosp = sqrt(q->w * q->w + q->v[2] * q->v[2]);
